...
class WyNDA:
  def __init__(self,
                 ...
                 P_state=None,
                 P_params=None):
    self.R_state = R_state if R_state is not None else 0.1 * np.eye(n_state, n_state)
    self.R_params = R_params if R_params is not None else 10 * np.eye(n_state, n_state)
    self.P_state = P_state if P_state is not None else 10 * np.eye(n_state, n_state)
    self.P_params = P_params if P_params is not None else np.eye(n_params, n_params)
  ...
  def run(self, input, wide_array, dt, base=None):
    if base is None :
